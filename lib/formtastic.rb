...
module Formtastic #:nodoc:
  ...
  class SemanticFormBuilder < ActionView::Helpers::FormBuilder
    ...
    protected
      ...
      # Take additional options - :as_stream, :skip_labels.
      # Example:
      #   f.input :created_at, :as => datetime, :skip_labels => true, :as_stream => true
      #   f.input :created_at, :as => time, :skip_labels => {:hour => true, :minute => true}
      # When :as_stream is set to true, it will not render the inputs as list.
      #  <fieldset>
      ...
      def date_or_datetime_input(method, options)
        ...
        date_inputs   = options.delete(:order) || i18n_date_order || [:year, :month, :day]
        ...
        (date_inputs + time_inputs).each do |input|
          ...
          if options[:"discard_#{input}"]
            ...
            label = ''
            if opts[:skip_labels].nil? or opts[:skip_labels] == false or (opts[:skip_labels].is_a? Hash and (opts[:skip_labels][input].nil? or opts[:skip_labels] == false))
              item_label_text = ::I18n.t(input.to_s, :default => input.to_s.humanize, :scope => [:datetime, :prompts])
              label << template.content_tag(:label, item_label_text, :for => input_id)
            end

            if options[:as_stream] then
              input_block = template.content_tag(:span,
                label <<
                template.send(:"select_#{input}", datetime, opts, html_options.merge(:id => input_id))
             )
            else
              input_block = template.content_tag(:li,
                label <<
                template.send(:"select_#{input}", datetime, opts, html_options.merge(:id => input_id))
              )
            end
            list_items_capture << input_block
          end
        end

        if options[:as_stream] then
          hidden_fields_capture <<
            self.label(method, options_for_label(options.merge(:label_for => input_ids.first)).merge(:for => options.delete(:label_for))) <<
            template.content_tag(:span, list_items_capture, :class => 'datetime')
        else
          hidden_fields_capture <<
          template.content_tag(:fieldset,
              template.content_tag(:legend,
                  self.label(method, options_for_label(options.merge(:label_for => input_ids.first)).merge(:for => options.delete(:label_for))), :class => 'label'
                ) <<
